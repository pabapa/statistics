...
func testNist() {
	...
	numacc1 := Float64{10000001, 10000003, 10000002}

	numacc2 := make(Float64, 1001)

	numacc3 := make(Float64, 1001)

	numacc4 := make(Float64, 1001)
...

	{
		mean := Mean(&lew)
		sd := Sd(&lew)
		lag1 := Lag1Autocorrelation(&lew)

		expected_mean := -177.435000000000
		expected_sd := 277.332168044316
		expected_lag1 := -0.307304800605679

		gsl_test_rel(mean, expected_mean, 1e-15, "lew Mean")
		gsl_test_rel(sd, expected_sd, 1e-15, "lew Sd")
...

	{
		mean := Mean(&lottery)
		sd := Sd(&lottery)
		lag1 := Lag1Autocorrelation(&lottery)

		expected_mean := 518.958715596330
		expected_sd := 291.699727470969
		expected_lag1 := -0.120948622967393
...

	{
		mean := Mean(&mavro)
		sd := Sd(&mavro)
		lag1 := Lag1Autocorrelation(&mavro)

		expected_mean := 2.00185600000000
		expected_sd := 0.000429123454003053
		expected_lag1 := 0.937989183438248
...

	{
		mean := Mean(&michelson)
		sd := Sd(&michelson)
		lag1 := Lag1Autocorrelation(&michelson)

		expected_mean := 299.852400000000
		expected_sd := 0.0790105478190518
		expected_lag1 := 0.535199668621283
...

	{
		mean := Mean(&pidigits)
		sd := Sd(&pidigits)
		lag1 := Lag1Autocorrelation(&pidigits)

		expected_mean := 4.53480000000000
		expected_sd := 2.86733906028871
		expected_lag1 := -0.00355099287237972
...

	{
		mean := Mean(&numacc1)
		sd := Sd(&numacc1)
		lag1 := Lag1Autocorrelation(&numacc1)

		expected_mean := 10000002.
		expected_sd := 1.
		expected_lag1 := -0.5
...

	{
		mean := Mean(&numacc2)
		sd := Sd(&numacc2)
		lag1 := Lag1Autocorrelation(&numacc2)
...

	{
		mean := Mean(&numacc3)
		sd := Sd(&numacc3)
		lag1 := Lag1Autocorrelation(&numacc3)

		expected_mean := 1000000.2
		expected_sd := 0.1
		expected_lag1 := -0.999
...

	{
		mean := Mean(&numacc4)
		sd := Sd(&numacc4)
		lag1 := Lag1Autocorrelation(&numacc4)

		expected_mean := 10000000.2
		expected_sd := 0.1
		expected_lag1 := -0.999
